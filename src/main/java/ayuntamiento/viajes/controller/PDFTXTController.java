...
import ayuntamiento.viajes.service.TXTService;
 ...
 * @version 1.3
 */
public class PDFTXTController extends BaseController implements Initializable {
    ...
    @FXML
    private Button printPhoneButton;

    private final static PDFService pdf;
    private final static TXTService txt;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        if (role != null && (role.equalsIgnoreCase("Admin") || role.equalsIgnoreCase("Superadmin"))) {
            printPhoneButton.setVisible(true);
        } else {
            printPhoneButton.setVisible(false);
        }
        
        sortCB.getItems().addAll("Nombre", "Fecha de Inscripción", "DNI"); 
                ...
                if (tripCB.getValue().getId() == 0) {
                    ...
                    if (travellerS.findByTrip(tripCB.getValue().getId()).isEmpty()) {
                        info("No existen viajeros registrados para el viaje "
                                + tripCB.getValue().getDescriptor(), false);
                    } else {
    ...
    /**
     * Metodo que imprime un txt con unicamente los teléfonos, o muestra el error según donde el usuario se
     * equivocó
     */
    @FXML
    private void printPhone() {
        ...
        if (isNotValidFileName()) {
            ...
            try {
                ...
                if (tripCB.getValue().getId() == 0) {
                    error(new ControledException("Para generar un txt primero especifique un viaje",
                    "PDFTXTController - printPDF"));
                } else {
                    if (travellerS.findByTrip(tripCB.getValue().getId()).isEmpty()) {
                        info("No existen viajeros registrados para el viaje "
                        ...
                        txt.printByTravel(namePDF.getText(), dirPDF.getText(),tripCB.getValue().getId());
                    }
                }
